...
def train(inDir, dataDir, seqName, seq_length, model,
    ...
    if model == 'RF':
        ...
        early_stopper = EarlyStopping(monitor='val_acc', patience=50,  mode='auto')
        ...
        checkpoint = ModelCheckpoint(filepath, monitor='val_accuarcy', verbose=1, save_best_only=True, mode='max')
        rm.model.fit(
                X_train,
                Y_train,
...
def main(argv):
    ...
    if (len(argv)==0):
        xmlName = './cnfgXMLs/NASNet33_lstm3.xml'
    else:
        xmlName = argv[0]
