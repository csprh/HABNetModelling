...
class ResearchModels():
    ...
    def lstm3(self):
        ...
        # Model.
        model = Sequential()
        model.add(TimeDistributed(Conv1D(filters=64, kernel_size=1, activation='relu'), input_shape=(None, self.input_shape[0], self.input_shape[1])))
        ...
        m#odel.add(MaxPooling1D(pool_size=2, input_shape = (self.input_shape[0], self.input_shape[1])))
        #model.add(Bidirectional(LSTM(2048, return_sequences=False,
        model.add(LSTM(512, return_sequences=False))
        model.add(Dropout(0.5))
